...
# Estado para las Figuras (Persistencia)
...
# Estado para el Zoom (La clave para que el botón funcione)
if 'zoom_uid' not in st.session_state: st.session_state.zoom_uid = "init_zoom"

# --- 3. CALLBACKS ---
def reset_figures():
    """Borra las figuras para recrearlas con datos nuevos (cambio de archivo)"""
    st.session_state.fig_time = None
    st.session_state.fig_freq = None
    # Reseteamos zoom también al cambiar archivo
...
def trigger_zoom_reset():
    """Cambia el ID del zoom para forzar a Plotly a resetear la cámara"""
    st.session_state.zoom_uid = str(uuid.uuid4())
...
def render_smart_player(audio_bytes, fs, unique_id):
    ...
    html_id = "persistent_player_v5"
    storage_key = f"t_{unique_id}"
...
with col_viz:
    ...
    with t1:
        ...
        if st.session_state.fig_time is None:
            ...
            # ACTUALIZAR DATOS EXISTENTES (Stateful)
            # Actualizamos Original (por si cambió el tramo de loop)
            st.session_state.fig_time.data[0].x = t_orig
            st.session_state.fig_time.data[0].y = v_orig
            # Actualizamos Procesada
            st.session_state.fig_time.data[1].x = t_proc
            st.session_state.fig_time.data[1].y = v_proc

        # APLICAR UIREVISION (La magia del zoom)
        # Asignamos el ID de zoom actual al layout
    ...
    # --- PESTAÑA FRECUENCIA (COMPARATIVA) ---
    with t2:
        # FFT Original
        ...
        if st.session_state.fig_freq is None:
            ...
            fig.update_layout(height=280, margin=dict(t=10,b=10,l=10,r=10), 
                            xaxis_type="log", legend=dict(orientation="h", y=1.1))
...
with col_play:
    ...
    # ID único para persistencia de tiempo
